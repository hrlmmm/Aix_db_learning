...
from controllers.dify_rest_api import DiFyRestApi
...
async def authenticate_user(username, password):
    ...
    with pool.get_session() as session:
        ...
        user = session.query(TUser).filter(TUser.user_name == username).first()
        if user and user.password:
            ...
            if user.password == password:
                return model_to_dict(user)
        return False
    # sql = f"""select * from t_user where user_name='{username}' and password='{password}'"""
...
async def query_user_list(page, size, name=None):
    ...
    with pool.get_session() as session:
        ...
        if name:
            query = query.filter(TUser.user_name.like(f"%{name}%"))

        total_count = query.count()
        total_pages = (total_count + size - 1) // size

        users = query.order_by(TUser.create_time.desc()).offset((page - 1) * size).limit(size).all()

        user_list = []
        for user in users:
            u_dict = model_to_dict(user)
            # if u_dict.get("create_time"):
            #     u_dict["create_time"] = u_dict["create_time"].strftime("%Y-%m-%d %H:%M:%S")
            # if u_dict.get("update_time"):
            #     u_dict["update_time"] = u_dict["update_time"].strftime("%Y-%m-%d %H:%M:%S")
...
async def add_user(username, password, mobile, role="user"):
    ...
    """
    with pool.get_session() as session:
        exist = session.query(TUser).filter(TUser.user_name == username).first()
        if exist:
...
async def init_super_admin():
    ...
    try:
        with pool.get_session() as session:
            ...
            # 这里简单检查用户名
            exist = session.query(TUser).filter(TUser.user_name == admin_name).first()
            if not exist:
