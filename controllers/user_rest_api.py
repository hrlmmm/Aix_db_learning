...
async def login(request: Request, body: LoginRequest):
    ...
    if user:
        ...
        token = await generate_jwt_token(user["id"], user["user_name"], role)
        return {"token": token}
    else:
        # 如果验证失败,返回错误信息
...
async def user_add(request: Request, body: AddUserRequest):
    ...
    :return:
    """
    return await add_user(body.user_name, body.password, body.mobile)
...
async def user_update(request: Request, body: UpdateUserRequest):
    ...
    :return:
    """
    return await update_user(body.id, body.user_name, body.mobile, body.password)
